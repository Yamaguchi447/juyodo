# 重要度の逆数を正規化し，その比率通りにZooming（シグモイド関数による補正を，各ステージごとに強める）
...
# シグモイド関数
def convert_scale(w, k):
    return 1 / (1 + np.exp(-1 * k * (w - 0.5)))


# initial_k=, k_increment= の部分に初期のシグモイド関数と各ステージごとの増分を指定
class SeqUD2(BaseSampler):
    def __init__(self, param_space, n_runs_per_stage, initial_k=1, k_increment=1, random_state=None):
        super().__init__()
    ...
    def _shrink_search_space(self):
        ...
        # 現在のステージに応じてkを計算
        current_k = self.initial_k + self.current_stage * self.k_increment

        # 重要度の逆数を計算
        inverse_importance = {k: 1.0 / v if v > 0 else float("inf") for k, v in importance.items()}
        ...
        for k in self.param_space.keys():
            ...
            scaled_inverse_importance = convert_scale(inverse_importance[k] / inverse_importance_sum, current_k)
            range_span = (self.upper_bounds[k] - self.lower_bounds[k]) * scaled_inverse_importance
