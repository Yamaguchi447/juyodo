# 重要度の比率通りにZooming（シグモイド関数による補正を，ステージごとに強める）
...
class SeqUD2(BaseSampler):
    ...
    def _shrink_search_space(self):
        ...
        for k in self.param_space.keys():
            # パラメータの重要度に基づいて範囲を縮小し，シグモイド関数でスケール
            importance_weight = importance[k] / importance_sum
            scaled_importance = convert_scale(importance_weight, current_k)
            range_span = (self.upper_bounds[k] - self.lower_bounds[k]) * scaled_importance
